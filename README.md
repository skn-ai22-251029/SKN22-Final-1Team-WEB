...
| 팀원 | 역할 | 소감 |
| --- | --- | --- |
| 최정환 | AWS EB 배포 및 운영 환경 관리, CI/CD 파이프라인 구축으로 배포 자동화, UI/UX 기획 및 화면 흐름 개선, 프론트엔드 구현 및           디자인 디테일 고도화, 인증/권한 기반 보안 로직 설계 및 적용|비전공자의인생 첫 웹배포 프로젝트로 어려운 부분이 많았지만 PM님과 팀원분들 덕분에 즐겁게 프로젝트 마무리 할 수 있어 좋았습니다.|
| 문승준 | 화면설계서, 챗봇용 NCS 학습자료 전처리, 프론트 표시용 API 응답 정비, 배포 환경 안정화, QA |처음배우는프로그래밍,미숙한 소통에 불구하고 끝까지 완주할 수 있도록 모두가 잘끌어주셨습니다. 참많은것을배웠습니다.|
| 이병재 | PM, 모델 파이프라인 구축, 데이터 크롤링, RAG 파이프라인 구축, DB설계, 문서작성, QA, FE/BE 수정  | PM으로서 부족한 점도 많았지만, 끝까지 믿고 같이 달려준 팀원들 덕분에 프로젝트를 무사히 마칠 수 있어 감사한 마음이 큽니다.|
| 장완식 | 데이터 전처리, 모델 파이프라인 구축, RAG 및 챗봇 구성, 모델 학습 및 평가, Redis 적용, 배포환경 설정, FE/BE 수정, 산출물 작성 | 생소한 도메인이라 어려운점도 있었지만, 막내인 제 의견을 존중하고 따라주신 덕분에 정말 재밌게 마무리 했습니다. 끝까지 믿고 함께 달려주셔서 진심으로 감사합니다! |
